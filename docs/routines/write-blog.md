...
その後、write-blog スキルを使って記事を 1 本執筆し、social.md 込みの PR を作成して終了する。
...
- GitHub CLI: `gh pr create` と、自分の PR のコメント・チェック読み取り
...
- 新規記事は 1 本以下
- PR URL、Notion 仮説リンク、experiment、検証結果を最終報告
- 自己承認・自己マージ・デプロイは行わない

## Registration prerequisites

- `docs/style-guide.md` と `facts.md` を HIRO が承認済みの内容で埋める
- Claude Code から Notion コネクタと GitHub CLIを利用可能にする
- GitHub 側の content-critic、CODEOWNERS、content-only 自動マージはこの
  リポジトリでは未接続。接続完了までは Routine が作った PR を人間がレビューする
